...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch(card) 
    ...
    case baron:
      return playBaron(state, handPos, choice1);
...
int playBaron(struct gameState* state, int handPos, int discardEstate)
{
	...
	{	
		...
		if (i >= 0) //Found an estate card!
		{
			state->coins += 4;	//Add 4 coins to the amount of coins
			discardCard(i, state->whoseTurn, state, 0);	// discard the estate card we found
...
{
	int temp = state->hand[state->whoseTurn][cardToTrash];  //store card type that we will trash

    if (state->hand[state->whoseTurn][cardToTrash] < copper || state->hand[state->whoseTurn][cardToTrash] >= gold)	// **intentional bug**: >= should be >
	{
	  return -1;
	}
		
    if (cardToTake > treasure_map || cardToTake < curse)
	{
	  return -1;
	}

    if ((getCost(state->hand[state->whoseTurn][cardToTrash]) + 3) >= getCost(cardToTake))	// **intentional bug**: >= should be >
	{
	  return -1;
	}

    gainCard(cardToTake, state, 2, state->whoseTurn);
	
    //discard card from hand
    discardCard(handPos, state->whoseTurn, state, 0);

    //discard trashed card
	discardCard(findCard(state, temp), state->whoseTurn, state, 1);			
...
int playTribute(struct gameState* state, int handPos)
{
    ...
    for (i = 0; i < 2; i++)
	{
		if (tributeRevealedCards[i] > copper && tributeRevealedCards[i] < gold)	// Treasure cards **intentional bug**: > should be >= and < should be <=
		{
			state->coins += 2;
		}
...
{
	int i, count;
	count = 0;		//used to check if player has enough cards to discard

    if (numCopies > 2 || numCopies < 0)
	{
	  return -1;				
	}
    if (cardToReturn == handPos)
	{
	  return -1;
	}

    for (i = 0; i < state->handCount[state->whoseTurn] && count < numCopies; i++)
	{
		if (i != cardToReturn && i == state->hand[state->whoseTurn][cardToReturn])
	    {
	      count++;
	    }
	}
    ...
    if (DEBUG) 
	{
		printf("Player %d reveals card number: %d\n", state->whoseTurn, state->hand[state->whoseTurn][cardToReturn]);
	}

    //increase supply count for choosen card by amount being discarded
    state->supplyCount[state->hand[state->whoseTurn][cardToReturn]] += numCopies;
    ...
    for (i = 0; i < state->numPlayers; i++)
	{
	  if (i != state->whoseTurn)
	    {
	      gainCard(state->hand[state->whoseTurn][cardToReturn], state, 1, i);	// **intentional bug**: gainCard(state->hand[state->whoseTurn][cardToReturn], state, 0, i);
	    }
	}

    //discard played card from hand
    discardCard(handPos, state->whoseTurn, state, 1);	// **intentional bug**:	discardCard(handPos, state->whoseTurn, state, 0);

    //trash copies of cards returned to supply
	for (i = 0; i < state->handCount[state->whoseTurn] && numCopies >= 0; i++)
	{
		if (state->hand[state->whoseTurn][i] == state->hand[state->whoseTurn][cardToReturn])
		{
...
{
	int i, j;
    state->numActions += 1;	// **intentional bug**: state->numActions++;
			
    if (choice == 1)		//+2 coins
	{
	  state->coins = state->coins + 2;	// **intentional bug**: state->coins += 2;
	  discardCard(findCard(state, minion), state->whoseTurn, state, 0);
	}	
    else	//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{	
		// all players discard hand and redraw if hand size > 4
		for (i = 0; i < state->numPlayers; i++)
	    {
			if (state->handCount[i] > 4 || i == state->whoseTurn)
			{
				//discard hand
